window.warRoom = function() {
    return {
        ...
        useServerTime: false, // Toggle for CET vs Server Reset Time
        ...
        updateClock() {
            const now = new Date();
            // Get CET Time (Standard for your grouping)
            ...
            // Calculate Server Time (Reset is 03:00 CET, so Server is CET - 3 hours)
            const serverDate = new Date(cetTime.getTime() - (3 * 60 * 60 * 1000));
            
            // Define Milestones based on your provided schedule
            // We find the 'Next' milestone to count down to
            let milestones = [
                { d: 1, h: 3, name: 'Grouping Phase', action: 'Scouting start.' },
                ...
                { d: 4, h: 3, name: 'Grouping Phase', action: 'Scouting start.' },
            ...
            if (!next) next = milestones[0]; // Loop to Monday
            ...
            // Display Time
            const displayDate = this.useServerTime ? serverDate : cetTime;
            this.currentRoundText = displayDate.toLocaleTimeString('en-GB', {hour: '2-digit', minute:'2-digit'}) + (this.useServerTime ? ' [SRV]' : ' [CET]');
        },
        ...
        async fetchData() {
            ...
            const fetchCSV = async (gid) => { 
                try { 
                    ...
                    return Papa.parse(t, {header:true, skipEmptyLines:true}).data.map(r => {
                        let c = {}; Object.keys(r).forEach(k => c[k.trim().toLowerCase().replace(/\s+/g,'')] = r[k] ? String(r[k]).trim() : ''); return c;
                    }); 
                } catch (e) { return []; } 
            };
        ...
        // --- GETTERS ---
        get filteredRefList() {
            if (!this.refSearch) return []; // Hide list until user types to save space
            const q = this.refSearch.toLowerCase();
            return this.alliances.filter(a => a.tag.toLowerCase().includes(q)).sort((a,b) => a.tag.localeCompare(b.tag));
        },
        ...
        getPassiveRate(tag) { 
            ...
            return c ? (Number(c.l1||0)*100)+(Number(c.l2||0)*200)+(Number(c.l3||0)*300)+(Number(c.l4||0)*400)+(Number(c.l5||0)*500)+(Number(c.l6||0)*600) : 0; 
        },
        ...
        copyScoutPrompt() { navigator.clipboard.writeText("Please provide: [Tag], [Stash], [Time]."); alert("Prompt Copied!"); },
