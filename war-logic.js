window.warRoom = function() {
    return {
        ...
        init() {
            ...
            this.fetchData();
            // Start clock and run once immediately
            this.updateClock();
        ...
        applyCalendar() {
            ...
            this.updateClock();
            alert("Calendar Applied! System Recalculated.");
        },
        ...
        updateClock() {
            ...
            if (this.round1Reset) {
                ...
                if (!isNaN(start)) {
                    ...
                    // 1. Calculate Week (1-4)
                    const calcWeek = Math.max(1, Math.min(4, Math.floor(diffDays / 7) + 1));
                    this.week = calcWeek;

                    // 2. Calculate Round (1-8)
                    // Mon(1), Tue(2), Wed(3) = First Round of week
                    // Thu(4), Fri(5), Sat(6), Sun(0) = Second Round of week
                ...
                this.currentRoundText = "Set Start Time";
            }

            // Phase Logic (Based on your schedule image)
            const hr = now.getHours();
            const d = now.getDay();
            if (d === 1 || d === 4) { 
                this.currentPhase = "Grouping Phase"; 
                this.phaseAction = "Brackets forming. No actions."; 
            } else if (d === 2 || d === 5) { 
                this.currentPhase = "Declaration Stage"; 
                this.phaseAction = "R4+ Declare War now!"; 
            } else if (d === 3 || d === 6) { 
                ...
                this.currentPhase = "Rest Time"; 
                this.phaseAction = "Preparing for next Monday."; 
            }
            this.phaseCountdown = now.toLocaleTimeString();
        },

        // ... [Keep the same fetchData, groupedForces, and export logic from previous versions] ...
        async fetchData() {
            this.loading = true;
            const cb = `&t=${Date.now()}`;
        ...
        get groupedForces() { const groups = {}; const allianceRanks = this.alliances.map(a => { const pList = this.players.filter(p => p.tag.toLowerCase() === a.tag.toLowerCase()); const maxTHP = pList.length > 0 ? Math.max(...pList.map(p => p.thp)) : 0; return { ...a, maxTHP }; }); allianceRanks.forEach(a => { if (!groups[a.server]) groups[a.server] = []; groups[a.server].push(a); }); Object.keys(groups).forEach(s => groups[s].sort((a,b) => b.maxTHP - a.maxTHP)); return groups; },
